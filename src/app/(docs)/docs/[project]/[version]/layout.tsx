...
import { getCachedNav, getCachedBranding } from '@/lib/cache';
...
export default async function DocsLayout({ children, params }: LayoutProps) {
  ...
  // Fetch nav and branding in parallel
  const [nav, cachedBranding, projectData] = await Promise.all([
    getCachedNav(project, version),
    getCachedBranding(project),
  ...
  const branding = (cachedBranding || projectData?.branding || {}) as BrandingSettings;
  const projectName = projectData?.name || project;
  const siteTitle = branding.siteTitle || projectName;
  ...
  return (
    <BrandingProvider branding={branding}>
      ...
      <div className="min-h-screen bg-white dark:bg-slate-900 flex flex-col">
          ...
          <div className="flex items-center justify-between px-4 py-3">
            <div className="flex items-center gap-2 sm:gap-4">
              ...
              >
                ...
                <span className="font-semibold text-slate-900 dark:text-slate-100 text-sm hidden sm:block">
                  {siteTitle}
                </span>
              </Link>
        ...
        {/* Footer */}
        <BrandedFooter branding={branding} projectName={projectName} />
      </div>
    </BrandingProvider>
  );
}
