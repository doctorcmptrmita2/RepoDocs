 ...
 * 
 * Displays the documentation footer with custom branding.
 * Shows footer text, links, social links, and "Powered by RepoDocs" badge.
 */
...
export function BrandedFooter({
  ...
  const hidePoweredBy = branding?.hidePoweredBy || false;
  const primaryColor = branding?.primaryColor || '#3B82F6';
  
  const currentYear = new Date().getFullYear();
  const defaultFooterText = `© ${currentYear} ${projectName}. All rights reserved.`;
  ...
  return (
    <footer className="border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 mt-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Social Links */}
        {socialLinks && (socialLinks.github || socialLinks.twitter || socialLinks.discord) && (
        ...
        {!hidePoweredBy && (
          <div className="mt-4 text-center">
            ...
            >
              ...
              <span className="font-semibold">RepoDocs</span>
            </Link>
          </div>
        )}
      </div>
